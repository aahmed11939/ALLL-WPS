...
name, the raw JSON payload, and ISO-8601 created/updated timestamps.
...
def list_projects() -> list[dict]:
    ...
    with _connect() as conn:
        rows = conn.execute(
            "SELECT slug, name, created_at, updated_at FROM projects ORDER BY updated_at DESC"
        ).fetchall()
    return [dict(r) for r in rows]


def create_project(data_json: str) -> dict:
    """
    ...
    Returns a dict with: slug, name, created_at, updated_at.
    """
    ...
    with _connect() as conn:
        ...
        conn.execute(
            "INSERT INTO projects (slug, name, data, created_at, updated_at)"
            " VALUES (?, ?, ?, ?, ?)",
            (slug, name, data_json, now, now),
        )
        conn.commit()

    return {"slug": slug, "name": name, "created_at": now, "updated_at": now}


def update_project(slug: str, data_json: str) -> dict:
    """
    ...
    Returns a dict with: slug, name, created_at (original), updated_at (new).
    """
    now = _now_utc()
    name = _extract_name(data_json)
    ...
    with _connect() as conn:
        row = conn.execute(
            "SELECT created_at FROM projects WHERE slug = ?", (slug,)
        ).fetchone()
        if row is None:
            raise KeyError(slug)
        created_at: str = row["created_at"]
        conn.execute(
            "UPDATE projects SET name=?, data=?, updated_at=? WHERE slug=?",
            (name, data_json, now, slug),
        )
        conn.commit()

    return {"slug": slug, "name": name, "created_at": created_at, "updated_at": now}
...
def load_project(slug: str) -> dict | None:
    ...
    with _connect() as conn:
        row = conn.execute(
            "SELECT slug, name, data, created_at, updated_at FROM projects WHERE slug=?",
            (slug,),
        ).fetchone()
    if row is None:
        return None
